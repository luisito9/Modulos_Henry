...
npm install
```
Dentro de nuestra carpeta `components` tendremos subcarpetas con nuestro archivo.js y .css para tener mas acomodado nuestro codigo.
...
```
La function `createStore` recibe argumentos. Primero le pasaremos nuestro `rootReducer` y despues nuestro Middleware para poder hacer peticiones asincronas en nuestro codigo.
...
<div style="text-align:center"><img src="src/img/show-movies.gif"/></div><br>
 ...
 En el caso de `addMovieFavorite` como parametro le pasamos un objecto con el titulo de la Pelicula y su ID, que mas adelante utilizaremos en el componente Favorites. Ej:
